...
NAME_UPDATED="false"
if [ "$ASSISTANT_NAME" != "Nano" ]; then
  log "Updating assistant name from Nano to $ASSISTANT_NAME"

  for md_file in groups/global/CLAUDE.md groups/main/CLAUDE.md; do
    if [ -f "$PROJECT_ROOT/$md_file" ]; then
      sed -i '' "s/^# Nano$/# $ASSISTANT_NAME/" "$PROJECT_ROOT/$md_file"
      sed -i '' "s/You are Nano/You are $ASSISTANT_NAME/g" "$PROJECT_ROOT/$md_file"
      log "Updated $md_file"
    else
